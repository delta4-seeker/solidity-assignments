...
contract Ballot {
 ...
 // Add a state variable startTime to record the voting start time.
  ...
  //Create a modifier voteEnded that will check if the voting period is over
    ...
    uint256 timeElapsed = block.timestamp - startTime ;  // calculate the time since voting started
        require( timeElapsed < 300 , "You are late buddy, voting deadline expired."); // if time is more than 5 minutes, revert.
        _;
    }
